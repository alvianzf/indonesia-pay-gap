...
- **Living-cost data (`living_costs_source.json`) is also province-level only.** KHL (Kemnaker's decent-living-needs figure, used by the Salary Affordability tool) covers all 38 provinces; BPS's poverty line covers 34/38. Numbeo's itemized cost-of-living breakdown covers 16 cities across 14 provinces (2 provinces — Jawa Timur, Jawa Barat — have 2 cities each; the region detail page picks the provincial-capital one) and is not used in any pay-gap calculation — it's raw, unreconciled, self-selected sample data (sample sizes range from hundreds of Numbeo entries for Jakarta down to single-digit contributor counts for e.g. Padang), kept in `merged.json.numbeoCities` for reference only and shown on detail pages for the provinces it covers. The file's `cpi_by_city` section remains too sparse/inconsistent to use at all and is intentionally not merged in.
